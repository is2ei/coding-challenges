...
vector<long> maximumPerimeterTriangle(vector<int> sticks) {

    sort(sticks.begin(), sticks.end());

    vector<long> v;
    long largest_perimeter = 0;
    for (int i = 0; i < sticks.size() - 2; i++) {
        ...
        if (largest_perimeter < perimeter) {
            ...
            tmp.push_back(sticks[i + 1]);
            tmp.push_back(sticks[i + 2]);
            v = tmp;
        }
    }
...
int main()
{
    ...
    vector<long> result = maximumPerimeterTriangle(sticks);
